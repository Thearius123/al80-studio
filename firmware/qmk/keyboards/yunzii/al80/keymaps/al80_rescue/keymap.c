...
/*
 * AL80 rescue path:
 * VIA command 0x0B -> STM32duino bootloader.
 *
 * This gives us a software recovery route that does not depend
 * on the key matrix as long as USB/Raw HID is alive.
 */
#ifdef VIA_ENABLE
#    include "via.h"
...
bool via_command_kb(uint8_t *data, uint8_t length) {
    if (length > 0 && data[0] == id_bootloader_jump) {
        bootloader_jump();
        return true;
    }
